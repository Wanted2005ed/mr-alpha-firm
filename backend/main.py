...
def analyze(symbol):
    ...
    direction='BUY' if close.iloc[-1]>ema20.iloc[-1]>ema50.iloc[-1]>ema200.iloc[-1] else 'SELL' if close.iloc[-1]<ema20.iloc[-1]<ema50.iloc[-1]<ema200.iloc[-1] else 'WAIT'
    score=40; reasons=[]
...
def manage_trades():
    ...
    for t in rows:
        ...
        if reason:
            risk=abs(t['entry']-t['sl']); r=(px-t['entry'])/risk if t['direction']=='BUY' else (t['entry']-px)/risk; c=db(); c.execute('UPDATE trades SET status="CLOSED",closed_at=?,exit=?,result_r=? WHERE id=?',(datetime.now(timezone.utc).isoformat(),px,r,t['id'])); c.commit(); c.close()
